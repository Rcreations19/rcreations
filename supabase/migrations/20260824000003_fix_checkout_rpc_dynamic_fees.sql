-- 1. Update process_checkout RPC to fetch delivery_charge and free_shipping_threshold from site_settings
...
BEGIN
  ...
  LOOP
    IF v_setting.key = 'delivery_charge' THEN
      v_delivery_charge := COALESCE((v_setting.value->>'text')::NUMERIC, 500);
    ELSIF v_setting.key = 'free_shipping_threshold' THEN
      v_free_shipping_threshold := COALESCE((v_setting.value->>'text')::NUMERIC, 10000);
    END IF;
  END LOOP;
